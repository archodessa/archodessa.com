...
function n__render_group_fotorama ($group, $myconf) {
  ...
  foreach ($group as $line) {
    ...
    if ($line['class'] == 'picture') {
	  ...
	  $thumb_filename = preg_replace('/\.jpg/', '.thumb.jpg', $thumb_filename);
	  $thumb_filename = preg_replace('/pictures/', '/pictures/thumbs/', $thumb_filename);
	  
	  $image_html = '<a href="'. $myconf['src-prefix'] . $filename .'">' .
		'<img src="'. $myconf['src-prefix'] . $thumb_filename .
